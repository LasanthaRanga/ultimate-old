 ...
 */
public class MobileAssessPayProcess {
...
//                    pvc.txt_Complete.setText(modle.Maths.get2String(compleeteTot));
    ...
    public int pay(int idOnPay, int assid, double cash, double chque) {
        ...
        if (b) {
            ...
            try {
                ...
                genarateRisitNoByOfficeAndOder(1, rn.getIdReceipt(), idOnPay, assid);
    ...
    public String genarateRisitNoByOfficeAndOder(int officeid, int idRecipt, int mobPayId, int assid) {
        ...
        try {
            ...
            ResultSet data = DB.getData("SELECT\n" +
                    ...
                    "FROM\n" +
                    "mobile_pay\n" +
                    "INNER JOIN mobile_tot ON mobile_tot.idMobTot = mobile_pay.mob_tot_id\n" +
                    "WHERE\n" +
                    "mobile_tot.idMobTot = '" + mobPayId + "' AND\n" +
                    "mobile_pay.app_id = " + assid);
...
//                    "YEAR(online_pay.date) = " + currentYear);
            ...
            if (data.last()) {
                no = data.getString("mobile_recipt_no");
                xx = data.getInt("oder");
                idMobilePay = data.getInt("idMobilePay");
            }
...
//            }
            ...
            conn.DB.setData("UPDATE `mobile_pay` SET `status`=3, `recipt_id`='" + idRecipt + "' WHERE `idMobilePay`=" + idMobilePay);
            conn.DB.setData("UPDATE `mobile_tot` SET `status`=3 WHERE `idMobTot`=" + mobPayId);
