...
import conn.DB;
import controller.assess.CreditAss;
...
public class CreditObj {
    ...
    Logger logger = Logger.getLogger(CreditObj.class.getName());
    ...
    public void cal(int idAssess, controller.assess.CreditAss pvc) {
        this.pvc = pvc;

        quater = GetInstans.getQuater();
        currentDay = quater.getCurrentDay();
        currentQuater = quater.getCurrentQuater();
        systemDate = quater.getSystemDate();

        systemDatesString = new SimpleDateFormat("yyyy-MM-dd").format(systemDate);
...
//                    pvc.txt_Complete.setText(modle.Maths.get2String(compleeteTot));
    ...
    public void viewPay() {
        ...
        double rawValue = totalAW + q1 + q2 + q3 + q4 + creditBalance;
//        BigDecimal value = rawValue.setScale(0, RoundingMode.UP);
        pvc.q4_tot.setText(modle.Round.roundToString(rawValue));
...
//                }
    ...
    public int pay() {
        ...
        if (b) {
            ...
            try {
                ...
                return Integer.parseInt(payy.toString());
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void viewId() {
        ...
        if (result.get() == ButtonType.OK) {
            ...
            GetInstans.getPrintBarcode().print(BarcodeStatic.customerName, BarcodeStatic.idRecipt + "", modle.Round.roundToString(BarcodeStatic.reTotal), BarcodeStatic.subject);
...
//        Session session = conn.NewHibernateUtil.getSessionFactory().openSession();
        ...
        try {
            ...
            List<Receipt> list = criteria.add(Restrictions.eq("applicationCatagory", (ApplicationCatagory) session.load(ApplicationCatagory.class, 2))).list();
            int size = list.size();
